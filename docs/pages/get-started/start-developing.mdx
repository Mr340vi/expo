...
const TripPlannerApp = () => {
  ...
  const popularMooringSites = [
    "The Bight, Norman Island",
    "Great Harbour, Jost Van Dyke",
    "Mosquito Bay’s Bio-luminescent Lagoon, Vieques",
    "Mosquito Bay Beach, Vieques",
    "Playa Tamarindo Beach, Culebra",
    "Bahia Flamenco Beach, Culebra",
    // More sites...
  ];

  const addDay = () => {
    setItinerary([...itinerary, { id: itinerary.length + 1, events: [] }]);
  };

  const addEvent = (dayId, eventType) => {
    setItinerary(
      itinerary.map((day) =>
        day.id === dayId
          ? { ...day, events: [...day.events, eventType] }
          : day
      )
    );
  };

  // Function to create the shareable text for itinerary
  const createShareableText = () => {
    let itineraryText = "My Trip Itinerary:\n\n";
    itinerary.forEach((day) => {
      itineraryText += `Day ${day.id}:\n`;
    ...
    itineraryText += "Popular Mooring Sites:\n";
    popularMooringSites.forEach((site) => {
      itineraryText += `  - ${site}\n`;
    });
  ...
  const shareItinerary = async () => {
    try {
      const itineraryText = createShareableText();
      await Share.share({
  ...
  return (
    <View style={{ flex: 1, padding: 20 }}>
      <Text style={{ fontSize: 24, fontWeight: "bold" }}>Trip Itinerary</Text>
      <FlatList
        data={itinerary}
        keyExtractor={(item) => item.id.toString()}
        renderItem={({ item }) => (
          <View style={{ padding: 10, borderBottomWidth: 1 }}>
            <Text style={{ fontSize: 18 }}>Day {item.id}</Text>
            ...
            <TouchableOpacity
              onPress={() => addEvent(item.id, "Snorkeling")}
              style={{ backgroundColor: "teal", padding: 5, marginTop: 5 }}
            >
              <Text style={{ color: "white", textAlign: "center" }}>Add Snorkeling</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => addEvent(item.id, "Dining")}
              style={{ backgroundColor: "orange", padding: 5, marginTop: 5 }}
            >
              <Text style={{ color: "white", textAlign: "center" }}>Add Dining</Text>
            </TouchableOpacity>
          </View>
        )}
      />
      ...
      >
        <Text style={{ color: "white", textAlign: "center" }}>Add Day</Text>
      </TouchableOpacity>
      ...
      >
        <Text style={{ color: "white" }}>Share Itinerary</Text>
      </TouchableOpacity>
    </View>
  );
};
